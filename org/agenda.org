...
- [ ] Missing tests? Complete this week for 50%
- [ ] Review of last week
...
1. What is data encapsulation in networking? Why is it important?
   #+begin_quote
   Data encapsulation is the process of wrapping data with
   protocol-specific headers and footers as it moves through the
   network stack. Each layer adds necessary metadata to ensure proper
   transmission, routing, and delivery. It is important because it
   allows modular communication, ensuring that higher layers (e.g.,
   web applications) do not need to manage lower-level details (e.g.,
   TCP, IP, Ethernet).
   #+end_quote
   #+attr_html: :width 200px:
   [[../img/ip_to_ethernet.png]]

2. Why can't a web browser directly send an HTML file over the
   network?
   #+begin_quote
   A web browser cannot send an HTML file directly because the network
   requires structured communication protocols. The HTML content must
   first be included in an HTTP response, which is then encapsulated
   in a TCP segment, wrapped inside an IP packet, and finally placed
   into an Ethernet frame for transmission.
   #+end_quote
   #+begin_example
   HTTP/1.1 200 OK
...
3. What role does TCP play in the encapsulation process when
...
4. How does a router handle an encapsulated HTTP response as it
   forwards it to the destination?
   #+begin_quote
   A router does not interpret the HTTP response itself. Instead, it
   examines the IP packet's destination address and determines the
   best next hop. It forwards the packet based on routing tables,
   stripping the previous Ethernet frame and encapsulating the IP
   packet in a new frame for the next network segment.
   #+end_quote
   #+begin_src bash :results output :exports both
   traceroute lyon.edu
   #+end_src

5. How can you observe real-time data encapsulation happening on your
   network?
   #+begin_quote
   You can use tools like =tcpdump= or Wireshark to capture packets in
   real-time. Running =sudo tcpdump -i eth0 -vv= allows you to see
   Ethernet frames, IP packets, and TCP segments, revealing the
