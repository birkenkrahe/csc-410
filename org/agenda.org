...
9. What's the first step to find a route from your PC to ~google.com~?
   #+begin_quote
   The URL ~google.com~ must undergo Network Address Translation (NAT)
   before packets can be sent to the corresponding IP address.
   #+end_quote

10. How can you see the IP routing table on Linux?
    #+begin_src bash :results output :exports both
      ip route show
    #+end_src
    #+begin_src bash :results output :exports both
