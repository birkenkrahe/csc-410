...
* DONE Week 14: Building a multiplex TCP client and server

- [X] Review and test: Networking a time-telling program
- [X] Lecture/practice: Forking and multiplexing
...
- A socket is a tool you use to do networking, like a telephone.

- A socket API is the interface (a set of functions) that lets you
  use sockets, including: ~bind(2)~, ~connect(2), ~send(2)~, ~recv(2)~,
  etc. This is the way you dial, answer, and speak into the telephone.

- An API can be about any application (task or service): Databases,
  graphics, file systems, not just networking. This could be
  telephones, text messages, or email - all communication interfaces.

- So a socket is not strictly speaking the same as an API.

- Here are properties of some common API frameworks that you'll see
  (Source: Kardell/Brouwer, 2025):
  #+caption: Source: Kardell/Brouwer 2025.
...
#+begin_example
  wget -O web.org tinyurl.com/web-codealong-1
#+end_example

And open it in Emacs!

* DONE Week 15: Building a TCP client +& Packet sniffing with Wireshark (?)

- [X] Last test available until Friday May 2nd.
- [X] 4th sprint review due in the week May 5-9.
- [X] Pick your presentation slot in Canvas now.

** DONE Final presentation
#+attr_html: :width 600px :float nil: 
[[../img/checklist.png]]

- [X] Rehearse with an audience
- [X] Test the technology before the presentation
- [X] Contact me if you have any questions 

** DONE Test preview/review

1. What happens to a program if it uses ~accept()~ and multiple clients
   try to connect?
   #+begin_quote
   Only one client can be served at a time; others must wait.
   #+end_quote

2. What system call creates a new process to avoid blocking the
   server?
   #+begin_quote
   The terribly dangerous ~fork()~ function (think "bomb").
   #+end_quote

3. What is the return value of ~fork()~ inside the child process?
   #+begin_quote
   0 - In the parent process, ~fork~ returns the PID of the newly
   created child process (a positive integer). This is efficient
   because you can write one program that tells you whether it's the
   parent or the child just by checking ~pid==0~. 

   | Return value | Process | Meaning                       |
   |--------------+---------+-------------------------------|
   |            0 | Child   | "I am the new process"        |
   |          > 0 | Parent  | "My child was created, PID=X" |
   |           -1 | Parent  | "Error, no child created"     |

   The value of =pid= is different depending on if you're in the parent
   or in the child process.
   #+end_quote

   #+begin_src C :main yes :includes <stdio.h> <unistd.h> :results output
     pid_t pid = fork();
   ...
   #+RESULTS:
   : Hello from parent with pid = 19738
   : Hello from child with pid = 0

4. What happens if you fork infinitely without setting a limit?
   #+begin_quote
   You overload the system by creating too many processes. To be
   exact: The shell process (parent process) that does the forking is
   overloaded.
   #+end_quote
   
5. What shell command limits the maximum number of user processes?
   #+begin_quote
   ~ulimit -u 100~ sets it to 100, ~ulimit -u~ gets the current value.
   #+end_quote

6. After forking, why should the child process call ~close(socket_listen)~?
   #+begin_quote
   Because the child is only serving a single client, not accepting
   new connections.
   #+end_quote

7. What function lets you monitor multiple sockets without blocking?
   #+begin_quote
   ~select()~
   #+end_quote

8. What must you do before adding sockets into an ~fd_set~?
   #+begin_quote
   Call ~FD_ZERO(&set)~ to clear the set where ~set~ is an ~fd_set~.
   #+end_quote

9. What does ~FD_ISSET(socket, &copy)~ check after ~select()~ where =copy=
   is a copy of ~fd_set~, and =socket= is the socket we're using?
   #+begin_quote
   It checks if ~socket~ is ready to read or write.
   #+end_quote

10. What happens if you set the ~timeout~ for ~select()~ to all zeros?
    #+begin_quote
    ~select()~ becomes non-blocking and returns immediately.
    #+end_quote

11. What kind of data structure is ~fd_set~?
    #+begin_quote
    It's a bit array (or bit mask) that tracks file descriptors: each
    bit tracks a file descriptor (socket, in our case), and the mask
    as a whole can be manipulated with macros like ~FD_ISSET~.
    #+end_quote

** DONE 3rd sprint review missing?
 #+attr_html: :width 600px :float nil:
 [[../img/third.png]]

** DONE Schedule your presentations in Canvas (or I'll do it for you)
 #+attr_html: :width 400px :float nil:
[[../img/schedule.png]]

** DONE 3rd sprint review
#+attr_html: :width 600px :float nil:
[[../img/third.png]]


** IN_PROGRESS Building a TCP client with multiplexing

Download the code along file: [[https://tinyurl.com/web-codealong-2][tinyurl.com/web-codealong-2]]
#+begin_example sh
  wget -O web2.org tinyurl.com/web-codealong-2
#+end_example

** TODO Wrap-up: What else we could have done

...if we had had 6 more weeks of term.

*Recommended:* Buy and finish the book by van Winkle, Hands-on
Networking Programming in C (2020). We've covered 100/450 pages.
#+attr_html: :width 400px :float nil: 
[[../img/vanWinkle.png]]

* DONE Week 16: Project presentations
#+attr_html: :width 400px:
[[../img/schedule.png]]
...
+ Get the *free book*: Beej's Guide to Network Programming Using
  Internet Sockets ([[https://beej.us/guide/bgnet/][beej.us/guide/bgnet/]]). Well written, very short.

+ Finish the *textbook for the course*: Hands-on Network Programming
  with C by Lewis van Winkle, Packt 2020).

+ Go deeper with C: UNIX Network Programming volume 1 ([[https://unpbook.com/][unpbook.com/]]),
  3rd edition.

+ Explore [[https://www.wireshark.org/][Wireshark]], a packet-sniffing tool

+ Do more [[eww:][Cisco Net Academy courses]] (Packet Tracer required)

+ Get the CCNA certification - work though [[https://www.manning.com/books/learn-cisco-network-administration-in-a-month-of-lunches][this book]] in a "month of
  lunches".

+ There is a nice [[https://www.udemy.com/user/davidbombal/][Udemy course by David Bombal]] where he shows the real
  networks with Switches, PCs, routers etc. and works with Packet
  Tracer and Wireshark. If you get it, wait for a discount period.
