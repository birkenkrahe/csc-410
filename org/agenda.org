...
* Week 8: More socket programming
...
- [ ] Exercises: Network interface listing with C

** DONE Mandatory Programming assignments
...
** DONE Optional programming assignments
...
** IN_PROGRESS Socket programming
...
- *Approach*: Comment each logical step, explaining purpose and
  ...
  #+begin_src C :tangle unix_list_commented.c
    ...
    int main() {
      ...
      while (address) {                     // Loop through all interfaces until NULL
      ...
      return 0;                // Exit successfully (zero)
    }
  #+end_src
...
- *Test*: Tangle (C-c C-v t), compile (`gcc -o unix_list_commented unix_list_commented.c`), run (`./unix_list_commented`).
...
** TODO Review: Network interface listing with C
...
2. How can you get your public IP address if you're behind a NAT?
   #+begin_quote
